...
public class Atencion extends AppCompatActivity {
    ...
    ImageView imageView_finish;//variable para la etiqueta de imagen
    String resultado_actividad = "", usuario = "";//varaibles string
    int n = 1, iniciar = 0, opc1 = 0, opc2 = 0, opc3 = 0, h = 0, puntos = 0, error = 0, intentos = 1, porcentaje = 0; // atributo del tipo entero
    float tStart = 0, elapsedSeconds = 0;//variables para obtener el tiempo

    @Override//Sobreescritura
    protected void onCreate(Bundle savedInstanceState) {//metodo constructor del activity
        super.onCreate(savedInstanceState);//llamado de la super clase
        setContentView(R.layout.activity_atencion);//para conectar con el archivo xml

        setTitle("Nivel 1 de Atencion");//titulo de la pantalla
        usuario = getIntent().getExtras().getString("idUsu");//OBTENEMOS EL NOMBRE DEL USUARIO
        imageButton_iniciar = (ImageButton) findViewById(R.id.imgbtn_iniciar); // inicializacion del botton
        imageButton_opc1 = (ImageButton) findViewById(R.id.imgbtn_opc1); // inicializacion del botton
        imageButton_opc2 = (ImageButton) findViewById(R.id.imgbtn_opc2); // inicializacion del botton
        imageButton_opc3 = (ImageButton) findViewById(R.id.imgbtn_opc3); // inicializacion del botton
        txt1 = (TextView) findViewById(R.id.textView22);// inicializacion texto
        txt2 = (TextView) findViewById(R.id.textView23);// inicializacion texto
        txt3 = (TextView) findViewById(R.id.textView24);// inicializacion texto
        txtmarcador = (TextView) findViewById(R.id.txtv_marcador);// inicializacion texto
        txtcronometro = (TextView) findViewById(R.id.text_cronometro1);// inicializacion texto
        imageView_finish = (ImageView) findViewById(R.id.imgv_finish);// inicializacion etiqueta para la imagen
        DesactivarBTN();
        ...
        imageButton_iniciar.setOnClickListener(new View.OnClickListener() {//INICIA EL METODO ACCION DENTRO DEL BOTON INDICADO
            ...
            public void onClick(View v) {//METODO DE LA ACCION DEL BOTON
                ...
                if (n <= 5) {//solo si es menor o igual a 5
                    ...
                    imageView_finish.setImageURI(null);//ESTO INDICA QUE NO GUARDARA NINGUNA IMAGEN
                    ...
                    oau = dataBaseAccess.SearchUnAudio(iniciar);//MANDAMOS A LLAMAR EL METODO DE BUSCAR AUDIO
                    Reproduccion(oau.getUbicacion());
                    ...
                    n++;//AUMENTAMOS LA VARIABLE PARA QUE SOLO TENGA 5 PREGUNTAS
                } else {
                    ...
                    imageView_finish.setImageURI(Uri.parse(oim.getUbicacion()));//muestra la imagen finish
                    txtmarcador.setText("Aciertos:" + puntos + ", Errores: " + error);//MUESTRA EL RESULTADO EN UN TEXT

                    porcentaje = 20 * puntos;//CALCULA EL PORCENTAJE DE ACIERTOS
                    ...
                    if (intentos == 1) {//INGRESA SOLO SI ES UNO PARA AGREGAR NUEVO REGISTRO
                        ...
                        resultado_actividad = dataBaseAccess.setInsertNuevoResultadoActividad(1, usuario, clasificacion.getClasificacion()+1,
                                "Nivel 1", intentos+"", "5", porcentaje + " %", 1);//CADA PARAMETRO LO INSERTAMOS
